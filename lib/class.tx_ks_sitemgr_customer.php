	...
	class tx_ks_sitemgr_customer {
		...
		function init() {
			if($this->customerId === null) {
				...
				return $this->customer = t3lib_BEfunc::getRecord('tx_kssitemgr_customer',$this->customerId);
			}
		}
		...
		function getCustomerForUser($uid) {
			$main_be_user = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
				'*',
				'tx_kssitemgr_customer',
				'deleted = 0 AND '.$GLOBALS['TYPO3_DB']->listQuery('main_be_user',$uid,'tx_kssitemgr_customer')
			);
			$admin_be_users = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
				'*',
				'tx_kssitemgr_customer',
				'deleted = 0 AND '.$GLOBALS['TYPO3_DB']->listQuery('admin_be_users',$uid,'tx_kssitemgr_customer')
			);
			$normal_be_users = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
				'*',
				'tx_kssitemgr_customer',
				'deleted = 0 AND '.$GLOBALS['TYPO3_DB']->listQuery('normal_be_users',$uid,'tx_kssitemgr_customer')
			);
			$customers = array();
